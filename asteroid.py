...
def compute_gravity_3body(v, t):
    uJ, _ = kep2cart(aJ, eJ, iJ, wJ, WJ, t, M0J, T0J)
    uJ = uJ[:, 0]
...
number_it = 365 * 500
step = 1
pos, speed = kep2cart(aA, eA, iA, wA, WA, 0, M0A, T0A)
v = np.hstack((pos[:, 0], speed[:, 0]))

# v = np.array([aA, 0, 0, 0, k / np.sqrt(aA), 0])

# -0.89, 5.09, -1.06, -7.5e-3, -9.5e-4, 1.72e-4

uJ, _ = kep2cart(aJ, eJ, iJ, wJ, WJ, 0, M0J, T0J)
# print(uJ)
...
for i in tqdm(range(number_it)):
    ...
    if i % 20 == 0:
        traj.scatter(v[0], v[1], c='C0', s=0.1)
        uJ, _ = kep2cart(aJ, eJ, iJ, wJ, WJ, i * step, M0J, T0J)
